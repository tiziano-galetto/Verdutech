...
class PDF extends FPDF
{
   ...
   function Header()
   {
      ...
      $this->Cell(100, 10, utf8_decode("REPORTE POR BUSQUEDA DE EMPLEADOS "), 0, 1, 'C', 0);
      $this->Ln(10);
...
if (!empty($Condiciones)) {
   ...
   if ($Stmt_busqueda && !empty($Tipos)) {
      ...
      $consulta_reporte_empleados = $Stmt_busqueda->get_result();
      $Stmt_busqueda->close();
   } else {
      $consulta_reporte_empleados = $conn->query($Sql_buscar);
   }
} else {
   $consulta_reporte_empleados = $conn->query($Sql_busqueda);
}

if ($consulta_reporte_empleados && $consulta_reporte_empleados->num_rows > 0) {
   ...
   $pdf->Cell(0, 10, utf8_decode("No se encontraron empleados"), 0, 1, 'C');
}
$pdf->Output('ReporteBusquedaEmpleados.pdf', 'I');
?>